# [MySQL索引背后的数据结构及算法原理（转载地址）](http://blog.codinglabs.org/articles/theory-of-mysql-index.html)
...
虽然这是一个货真价实的索引，但是实际的数据库系统几乎没有使用二叉查找树或其进化品种红黑树（red-black tree）实现的，原因会在下文介绍（组要是树层数太多，导致磁盘读取效率低下）。
...
一般来说，B+Tree比B-Tree更适合实现外存储索引结构，具体原因与外存储器原理及计算机存取原理有关，将在下面讨论(主要是不带data导致数据比较小，一个扇区可以存下更多的key)。
...
>下面是一个联合索引的数据结构图，取自博客[MySQL之索引原理与慢查询优化](https://www.cnblogs.com/clschao/articles/10049133.html)，可以先看一下联合索引的结构和大概原理，便于后面的理解。
>
>来看一下联合索引内部的结果。从本质上来说，联合索引就是一棵B+树，不同的是联合索引的键值得数量不是1，而是>=2。接着来讨论两个整型列组成的联合索引，假定两个键值得名称分别为a、b
>
>![联合索引结构图](https://images2018.cnblogs.com/blog/1036857/201711/1036857-20171126004856453-1491949427.png)
>
...
## 参考链接
>[MySQL之索引原理与慢查询优化](https://www.cnblogs.com/clschao/articles/10049133.html)
>
