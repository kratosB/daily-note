...
3. [集群模式](https://mp.weixin.qq.com/s?__biz=MzkzMDI1NjcyOQ==&mid=2247487789&idx=1&sn=7f8245f8b4e4a98aa0a717011f7b7e24&chksm=c27c531bf50bda0da3bcc325b131dac2553eb508fed4175ab1d883fb4946557fb91053ddb525&scene=178&cur_album_id=1918295695426404359#rd `Redis 高可用篇：Cluster 集群能支撑的数据有多大？`)
   ...
   3. 客户端如何定位数据所在实例。
      ...
      4. hash slot与redis实例对应关系改变。
         ...
         3. 集群提供了重定向机制。客户端给实例发送请求，实例没有对应数据，实例会告诉客户端将请求发给其他实例。
         4. 方案：
...
2. 先更新数据库，再删缓存（其实跟延迟双删的后半部分一样了）
   1. 缺点
      1. 更新完了，删缓存失败，数据不一致。
      2. 有延迟。
      3. 更新的同时，有人查了，然后删完缓存之后，查的人更新了缓存。
   2. 方案
      1. job，消息队列来做，复杂+有延迟。
      2. binlog来做，延迟还是有，但是延迟很少，就跟mysql主从一样，能忍。
      3. 查通常比更新快，第三种情况很少会发生。
3. 设置过期时间（最终一致性），（以防万一）
4. 删缓存，而不是更新缓存。
   1. 更新的缺点
      1. 因为更新了不一定会用上，白搞。
      2. 更新更容易出现脏数据的情况（并发更新）。
   2. 删除的缺点
      1. 会有一次cache miss
   3. 所以根据读多写少，或者读少写多，可以用不同的策略。
      1. 读多写少，以及热key可以用更新。
      2. 读少写多，用删除。
...
1. 底层就是hash表+hash桶（桶内部是链表，hash冲突就是用链表解决）。
2. 为了保持高效，redis会做rehash（增加hash桶，减少冲突）。
3. redis默认使用两个全局hash表，以提高rehash效率。（一个当前使用，一个扩容）
   1. 给hash表2（备用表）分配更大空间
   2. 将hash表1（当前表）的数据映射到hash表2
   3. 释放hash表1的空间
